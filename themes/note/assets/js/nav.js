function nav() {
    ...
    section.onmouseover = function () {
      Array.from(navSections).forEach((element) => {
        document.getElementById(
          `sub-pages-container-${element.id}`
        ).style.display = "none";
      });
      ...
      document.getElementById("gb-footer").classList.add("blurPage");
    };
  }
  
  const subPagesContainer = document.getElementsByClassName(
    "sub-pages-container"
  );
  // 遍历这些元素
  for (let div of subPagesContainer) {
    ...
    div.onmouseout = function () {
      ...
      document.getElementById("gb-footer").classList.remove("blurPage");
    };
  }
